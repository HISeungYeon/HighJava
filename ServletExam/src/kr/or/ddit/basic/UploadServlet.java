...
public class UploadServlet extends HttpServlet {
	...
	// 메모리 임계크기, 메모리의 한계점 (이 크기가 넘어가면 레파지토리 위치에 임시파일로 저장됨)
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		*/
		...
		// Multipart 파싱 전 파라미터 정보 조회 해 보기
		...
		if(ServletFileUpload.isMultipartContent(req)) { //?
			...
			System.out.println("임시저장위치 : " + System.getProperty("java.io.tepdir"));
			...
			try {
				...
				if(formItems != null && formItems.size() > 0 ) { // 파일인 경우,,
					for(FileItem item : formItems) {
						if(!item.isFormField()) {
							...
							System.out.println("업로드 완료된 => 파일명 : " + fileName);
						}else { // 폼 데이터인 경우..
							// 폼필드의 값이 한글인 경우에 해당하는 문자열을 적절히 변환을 해주어야 한다.
