...
public class T05ServletCookieTest extends HttpServlet {
	/*
	 * 쿠키에 대하여! (웹서버와 브라우져는 애플리케이션을 사용하는 동안 필요한 쿠키를 통해 공유하여 상태를 유지함. 
	 * 1. 구성요소? - 이름 - 값 - 유효시간(초) - 도메인 : ex) www.somehost.com, .somehost.com - 경로 : 쿠키를 공유할 기준경로를
	 * 지정한다. (지정하지 않으면 실행한 URL의 경로부분이 사용됨.)
	 * 
	 * 2. 동작방식 - 쿠키 생성 단계 : 생성한 쿠키를 응답데이터의 헤더에 저장하여 웹브라우져에 전송한다. - 쿠키 저장 단계 : 웹브라우져는
	 * 응답데이터에 포함된 쿠키를 쿠키저장소에 보관한다. - 쿠키 전송 단계 : 웹브라우져는 저장한 쿠키를 요청이 있을때마다 웹서버에
	 * 전송한다.(삭제전까지,,,) 웹서버는 브라우져가 전송한 쿠키를 사용하여 필요한 작업을 수행한다.
	 */
...
//		setCookieExam(req, resp); // 쿠키  설정 예제
	...
	private void setCookieExam(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		 ...
		 * 쿠키 정보 설정하는 방법
		 * 
		 * 1. 쿠키 객체 생성하기. 사용불가문자(공백, []()=,"/?@:;) => Cookie cookie = new Cookie("키값",
		 * "value값"); 쿠키값은 사용불가문자를 제외한 나머지 출력가능한 아스키 문자 사용 가능함. = 이외의 값 (예를들면 한글)을 사용
		 * 시에는 URLEncoder.encode() 사용하여 처리함.
		 * 
		 * 2. 쿠키 최대 지속시간을 설정한다. (초단위) => 지정하지 않으면 브라우져 종료시 삭제됨.
		 * cookie.setMaxAge(60*60*24); //24 시간
		 * 
		 * 3. 응답헤더에 쿠키객체를 추가한다. response.addCookie(cookie);
		 * 
		 * => 출력버퍼가 플러시 된 이후에는 쿠키를 추가할 수 없다. (응답헤더를 통해서 웹브라우져에 전달하기 때문에..)
		 */
